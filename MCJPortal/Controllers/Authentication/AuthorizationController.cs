...
namespace MCJPortal.Controllers.Authentication
{
    ...
    public class AuthorizationController : Controller
    {
        ...
        public async Task<IActionResult> Connect(OpenIdConnectRequest connectRequest)
        {
            ...
            if (connectRequest.IsPasswordGrantType())
            {
                ...
                // proceed only if Admin or Support
                if (await _userManager.IsInRoleAsync(user, Roles.Admin.ToString()) || await _userManager.IsInRoleAsync(user, Roles.User.ToString()))
                {
            ...
            throw new Exception("The specified grant type is not supported.");
        }
        ...
        async Task UpdateUserOnLogin(ApplicationUser user)
        {
            user.DateLastAccessed = DateTime.UtcNow;
